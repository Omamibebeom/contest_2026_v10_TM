...
| b | 隨機位置放置板（每次位置不同） | 送 GET 問座標，照回覆去夾 | 開場拍快照，算出每件的手臂座標 | `$x,y,color`（照 `PICK_ORDER` 的顏色順序） |
...
```
把物件放到鏡頭下，拉六條滑桿，直到「mask」視窗只剩物件是白的、背景全黑。按 `s`，輸入顏色名（小寫），存檔。每個顏色都做一次（放置板和隨機板的顏色全部存在同一個檔）。紅色要把 H min 拉得比 H max 大，畫面會顯示 HUE-WRAP。按 `n` 可以叫回已存的顏色修改，`q` 離開。
